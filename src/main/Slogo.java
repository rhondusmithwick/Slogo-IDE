...
public class Slogo {
	...
	public void newView() {
		...
		currView = new View(globalProperties, turtleDispDimension, this,
				((TurtleController) currController).getTurtlesMap());
	...
	private void bindProperties() {
		...
		controllerProperties.parallelStream().forEach(this::findTwin);
	}
	...
	private void findTwin(SimpleStringProperty cProp) {
		...
		Predicate<SimpleStringProperty> shouldBind = (p) -> Objects.equals(p.getName(), cName);
		viewProperties.parallelStream().filter(shouldBind).findFirst()
